...
DSpark weights live in the target checkpoint's ``mtp.*`` namespace, but the
draft path is a block drafter rather than the serial MTP module. Selected
target hidden states are projected into the draft attention context before the
...
class DeepseekV4DSparkModel(nn.Module):
    ...
    def _project_shared_kv(
        ...
        k_nope, k_pe = kv.split(
            [attn.nope_head_dim, attn.rope_head_dim],
            dim=-1,
        )
...
class DSparkDeepseekV4ForCausalLM(
    ...
    """Ascend DeepSeek V4 block-draft model loaded from ``mtp.*`` weights."""
    ...
    def load_weights(
        ...
        for checkpoint_name, loaded_weight in weights:
            ...
            if name == "embed.weight" and not self.has_own_embed_tokens:
                ...
                if mapped_name is None:
                    ...
                    # weights. Only the explicitly namespaced mtp weights belong
                    # to this draft model.
                    continue
                name = mapped_name
        ...
        optional_shared_params: set[str] = set()
        if not self.has_own_embed_tokens:
            optional_shared_params.update(name for name in params_dict if name.startswith("model.embed_tokens."))
        if not self.has_own_lm_head:
            optional_shared_params.update(name for name in params_dict if name.startswith("lm_head."))
    ...
    def _get_checkpoint_parameter(
        ...
        try:
            ...
            raise ValueError(
                "Unexpected DSpark checkpoint weight "
                f"{checkpoint_name!r}; mapped parameter {parameter_name!r} "
                "does not exist in the Ascend draft model."
            ) from exc
    ...
    def _remap_dspark_name(self, name: str) -> str | None:
        ...
        if stage >= self.model.num_dspark_layers:
            raise ValueError(
