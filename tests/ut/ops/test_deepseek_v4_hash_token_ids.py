...
def test_decoder_passes_same_token_tensor_to_mlp() -> None:
    ...
    layer.hc_attn_fn = torch.empty(1)
    layer.hc_attn_scale = torch.empty(1)
    layer.hc_attn_base = torch.empty(1)
    layer.hc_ffn_fn = torch.empty(1)
    layer.hc_ffn_scale = torch.empty(1)
    layer.hc_ffn_base = torch.empty(1)
