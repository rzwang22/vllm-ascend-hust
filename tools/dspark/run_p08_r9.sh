...
P08_BASELINE=6e0e536540cac92b8ad45b2d0ee0ff77b6d0a470
P08_PLUGIN_SHA=${1:-}
P08_DATA=${2:-/workspace/dspark-results/m2_5a-p08-r8-diagnostic.nmRC4J/input-dataset.jsonl}
P08_FIRST_EPOCH=${3:-60}
P08_LAST_EPOCH=${4:-80}
P08_DATA_SHA=6a2f629a5b5c9bbd9a3058b7a450fc18b2332f4699047f164cdde6a33b58d053
P08_CHECKS=$P08_PLUGIN/tools/dspark/p08_r9_checks.py
P08_PORT=29888
P08_OUT=
P08_STAGE=setup
...
p08_graph() {
    ...
    # Capture the exact expanded benchmark command as well as the script/arguments.
    set -x
    VLLM_PORT="$P08_PORT" MASTER_PORT="$P08_PORT" python "$P08_PLUGIN/tools/dspark/benchmark_dspark_acceptance.py" \
        ...
        --dspark-nan-diagnostic-dir "$P08_OUT/rank-diagnostics" \
        --dspark-nan-replay-window "$P08_FIRST_EPOCH" "$P08_LAST_EPOCH" --result-json "$P08_OUT/graph-b4-diagnostic.json"
}

p08_main() {
    mkdir -p /workspace/dspark-results || return 1
    P08_OUT=$(mktemp -d /workspace/dspark-results/m2_5a-p08-r9-diagnostic.XXXXXX) || return 1
    ...
    cp -- "$P08_CHECKS" "$P08_OUT/p08_r9_checks.py" || return 1
    cp -- "$P08_PLUGIN/tools/dspark/p08_r8_checks.py" "$P08_OUT/p08_r8_checks.py" || return 1
    printf 'PLUGIN_SHA=%s\nINPUT_PATH=%s\nFIRST_EPOCH=%s\nLAST_EPOCH=%s\n' "$P08_PLUGIN_SHA" "$P08_DATA" "$P08_FIRST_EPOCH" "$P08_LAST_EPOCH" > "$P08_OUT/invocation.txt"
    P08_STAGE=dataset
    ...
    p08_logged focused python -m pytest -q -ra \
        tests/ut/test_dspark_replay_diagnostics.py tests/ut/test_p08_r9_runner.py \
...
if test -n "$P08_OUT"; then
    ...
    p08_logged diagnostics python "$P08_CHECKS" diagnostics "$P08_OUT" "$P08_GRAPH_RC" "$P08_RESULT_RC"
    P08_DIAG_RC=$?
