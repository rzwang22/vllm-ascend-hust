...
# Run in the same initialized CANN/custom-op container as P08-R4.
set -u
set -o pipefail
...
P08_BASELINE=9c0d39237938b7ba26ddcb1fa08bdda1a54c4b45
...
p08_source_gate() {
    ...
    test "$(git -C "$P08_CORE_REPO" rev-parse HEAD)" = "$P08_CORE_SHA" || return 1
...
print(torch._C._dispatch_dump_table('aten::_assert_async.msg'))
PY
}

p08_main() {
    mkdir -p /workspace/dspark-results || return 1
    P08_OUT=$(mktemp -d /workspace/dspark-results/m2_5a-p08-r5.XXXXXX) || return 1
    ...
    VLLM_PORT=29865 MASTER_PORT=29865 python "$P08_PLUGIN_REPO/tools/dspark/benchmark_dspark_acceptance.py" \
