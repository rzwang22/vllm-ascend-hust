...
def _load_run(
    ...
    output_tokens = sum(int(record["output_token_count"]) for record in critical)
    decode_tokens = sum(int(record["performance"]["decode_output_token_count"]) for record in critical)
    prefill_seconds = sum(float(record["performance"]["prefill_latency_seconds"]) for record in critical)
    decode_seconds = sum(float(record["performance"]["decode_latency_seconds"]) for record in critical)
    inference_seconds = sum(float(record["performance"]["inference_latency_seconds"]) for record in critical)
    accepted_tokens = sum(int(record["performance"]["accepted_draft_token_count"]) for record in critical)
    scheduled_tokens = sum(int(record["performance"]["scheduled_draft_token_count"]) for record in critical)
    verification_count = sum(int(record["verification_count"]) for record in critical)
    ...
    return {
        ...
        "target_forward_count": sum(int(record["target_forward_count"]) for record in critical),
        "verification_count": verification_count,
        "proposal_generated_count": sum(int(record["proposal_generated_count"]) for record in critical),
        "proposal_installed_count": sum(int(record["proposal_installed_count"]) for record in critical),
        "proposal_consumed_count": sum(int(record["proposal_consumed_count"]) for record in critical),
        "accepted_draft_token_count": accepted_tokens,
        "scheduled_draft_token_count": scheduled_tokens,
        "average_accepted_tokens_per_verification": accepted_tokens / verification_count if verification_count else 0.0,
        "draft_token_acceptance_rate": accepted_tokens / scheduled_tokens if scheduled_tokens else 0.0,
        "scheduler_seconds": sum(float(record["performance"]["scheduler_seconds"]) for record in critical),
        "scheduler_update_seconds": sum(
            float(record["performance"]["scheduler_update_seconds"]) for record in critical
        ),
        "model_execute_host_seconds": sum(
            float(record["performance"]["model_execute_host_seconds"]) for record in critical
        ),
        "sample_materialize_seconds": sum(
            float(record["performance"]["sample_materialize_seconds"]) for record in critical
        ),
        "draft_install_seconds": sum(float(record["performance"]["draft_install_seconds"]) for record in critical),
        "spec_decode_proposer_latency_seconds": sum(
            float(record["performance"]["spec_decode_proposer_latency_seconds"]) for record in critical
        ),
        "spec_decode_verification_latency_seconds": sum(
            float(record["performance"]["spec_decode_verification_latency_seconds"]) for record in critical
        ),
...
def summarize_performance(
    ...
    *,
    ...
    if dspark_decode <= 0 or dspark_inference <= 0:
    ...
    return {
        ...
        "speedup": {
            "primary_decode_median": target_decode / dspark_decode,
            "end_to_end_inference_median": target_inference / dspark_inference,
        },
