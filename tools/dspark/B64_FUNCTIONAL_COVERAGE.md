...
第一阶段已由 hBoDUBTA / XM02ngWZ 归档独立核验通过并冻结：12点、328请求、数值/归属/FULL与自然退出均通过。
后续唯一合成扩展为 [第二阶段](B64_FUNCTIONAL_PHASE2.md) 的2048输入×16/32/64请求7点；
通过后转入单独真实文本验证，不继续遍历剩余矩阵。以下保留第一阶段入口与原十点历史依据。
...
第一阶段后有114个原矩阵point未覆盖，这不是待完成清单。根据已通过结果，第二阶段只选2048输入的
16/32/64请求7点，通过后结束合成扩展。其余形状、完整矩阵和性能比较不自动安排。
...
第一阶段12点真实NPU功能覆盖已通过并冻结，详见第二阶段文档的独立审计。原十点已通过状态不受影响。
