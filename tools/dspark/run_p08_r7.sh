...
# Run in the same initialized CANN/custom-op container as P08-R6.
set -u
set -o pipefail
...
P08_BASELINE=81d8376f0b7e56625454581cd2ebd2a9150cda6d
...
assert callable(getattr(worker_extension, 'dspark_benchmark_replay_snapshot'))
...
p08_main() {
    ...
    P08_OUT=$(mktemp -d /workspace/dspark-results/m2_5a-p08-r7.XXXXXX) || return 1
    ...
    VLLM_PORT=29867 MASTER_PORT=29867 python "$P08_PLUGIN_REPO/tools/dspark/benchmark_dspark_acceptance.py" \
...
g = r['graph_execution']
assert g['replay_evidence_status'] == 'available'
assert g['source'] == 'mrv2_successful_execute_model_full_replay'
assert r['measured_graph_replay_count'] > 0
m = g['measured_runtime']
assert m['source'] == g['source']
assert m['graph_replay_count'] == r['measured_graph_replay_count']
assert sorted(w['rank'] for w in m['workers']) == list(range(8))
assert all(w['records'] == m['records'] and w['graph_replay_count'] == m['graph_replay_count']
           and w['failed_execution_count'] == 0 for w in m['workers'])
assert sum(x['count'] for x in m['records']) == m['graph_replay_count']
assert all(x['runtime_mode'] == 'FULL' and x['count'] > 0 for x in m['records'])
assert r['measured_eager_fallback_count'] is None
assert g['eager_fallback_evidence'].startswith('unavailable:')
assert len(g['boundary_snapshots']) == 3
